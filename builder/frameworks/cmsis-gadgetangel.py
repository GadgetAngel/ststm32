...
CMSIS_DIR = platform.get_package_dir("framework-cmsis-gadgetangel")
CMSIS_DEVICE_DIR = platform.get_package_dir("framework-cmsis-gadgetangel-" + mcu[0:7])
...
def prepare_startup_file(src_path):
    ...
    if not os.path.isfile(startup_file) and os.path.isfile(startup_file[:-2] + ".s"):
        # //ga save all files expect for startup_stm32f100xx.s file
        if (startup_file[:-2] + ".s") != "startup_stm32f100.s":
